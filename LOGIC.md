...
Two pricing models are supported per customer (`pricing_model: flat` is the default).

### Flat pricing (`pricing_model: flat`)
...
### Banded pricing (`pricing_model: banded`)

```
base           = rate_matrix[mile_band(miles)][weight_band(weight_lbs)]
revenue_calc   = base + stop_count × rate_per_stop
revenue        = max(revenue_calc, minimum_charge)
```

The customer config provides:
- `mile_bands` — upper bounds, ascending. e.g. `[50, 150, 300, 500]` → 5 rows: ≤50, ≤150, ≤300, ≤500, >500.
- `weight_bands` — same shape, in lbs.
- `rate_matrix` — 2D array of total dollars, shape `(len(mile_bands)+1, len(weight_bands)+1)`.
- Optional `rate_per_stop` and `minimum_charge` add-ons (default 0).

**Band-lookup rules:**
- **Upper-bound inclusive.** Route at exactly 50 mi → first row. Route at 50.01 mi → second row.
- **Above the largest band:** clamp to the last row/column. The top tier is treated as "and up."
- **NaN miles:** revenue = NaN (same as flat-rate behavior when M3PL miles are missing).
- **NaN / 0 weight:** falls into the lowest weight column (column 0).
- **Mismatched matrix shape** (rows ≠ `len(mile_bands)+1` or any row's cols ≠ `len(weight_bands)+1`): `lookup_banded_rate` raises `ValueError` at compute time; the dashboard editor blocks save when shape is wrong.
...
4. `pricing_model` defaults to `"flat"` if missing on either default or customer entry.
5. Apply the appropriate formula based on `pricing_model`.

**Override:** Edit `config/customer_rates.yaml` directly, or use the Rate Matrix dashboard page (manual editor for flat customers, expander editors for banded customers, or the **Banded Rate Import Wizard** to ingest an arbitrary Excel rate sheet).

**Code:** `datascrubb/kpi/revenue.py::compute_route_revenue`, `rate_for`, `load_rate_matrix`, `lookup_banded_rate` · **Table:** `route_revenue` (now includes `pricing_model` and `revenue_banded` columns) · **Page:** Cost vs Revenue
