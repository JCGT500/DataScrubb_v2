...
class TelemetryStop(Base):
    ...
    min_evap_delta = Column(Float)
    max_evap_delta = Column(Float)
    da1_present_pct = Column(Float)
    setpoint_compliance_pct = Column(Float)
    defrost_event_count = Column(Integer)
    max_cargo_temp = Column(Float)
    bulkhead_seal_index = Column(Float)
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
